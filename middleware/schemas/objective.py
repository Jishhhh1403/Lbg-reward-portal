...
These models drive the AI-generated SDUI for the modal wizard. Each stage of
the wizard returns a generic list of SDUI components (the same contract used
by the Rewards Dashboard) which the frontend renders through a component
registry. This lets the AI compose the screen itself, rather than the frontend
mapping a typed payload onto a hardcoded screen skeleton.

Stage names mirror the wizard steps:
  summary       -> Screen 1b
...
class ObjectiveScreenPayload(BaseModel):
    """Typed content captured per stage before it is composed into SDUI."""
